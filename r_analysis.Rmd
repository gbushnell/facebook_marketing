...
set.seed(13)
data <- read.table("wp_fb_sales.csv", header = TRUE, sep = ",")
...
#Top Sales is 6k, which is well above the next highest at over 48k, remove all higher than 48k
data_PY = data_PY[data_PY$Sales < 5000,]
...
```{r}
model2 <- lm(data=data_PY, log(Sales) ~ Post)
summary(model2)
gvlma(model2)
```
...
We can see that our t-statistic is 2.701, meaning if we believed there was no statistical difference between the two means (that they both were equal), we'd expect to see this type of result .8% of the time. We can safely conclude that there is a statistically significant differnce between days with posts and days without. 
